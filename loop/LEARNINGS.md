T1: Anchored the runner slide scale from the top and made the runner crop explicit, so the 1440 verifier keeps crown clearance during the breathe animation.
T1: Moved the rust block to a right-inset in-panel rectangle instead of the old left offset.
Next iteration: T2 is hero copy only. Avoid broad page copy edits until T7.
T2: Replaced the two-sentence explanatory hero subhead with one restrained sentence, keeping headline, CTA labels, and location/membership microcopy intact.
T2: DOM count is 29 visible hero words, so the fixed CTA and microcopy budget leaves only 11 words for the subhead.
Next iteration: T3 is the capability-curve slider port. Keep hero copy untouched unless a verifier explicitly fails it.
